...
/** Fees smaller than this (in ufdreserve) are considered zero fee (for relaying and mining)
...
CAmount GetBlockValue(int nHeight)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET){
        ...
        if ( nHeight == 0 ) {
            nSubsidy = 5246397 * COIN; // premine
        } else if (nHeight <= 10000) {
            nSubsidy = 2 * COIN;
        } else if (nHeight > 10000 && nHeight <= 86400) {
            nSubsidy = 3 * COIN;
        } else if (nHeight > 86400 && nHeight <= 216000) {
            nSubsidy = 4 * COIN;
        } else if (nHeight > 216000 && nHeight <= 388800) {
            nSubsidy = 5 * COIN;
        } else if (nHeight > 388800 && nHeight <= 518400) {
            nSubsidy = 6 * COIN;
        } else if (nHeight > 518400 && nHeight <= 691200) {
            nSubsidy = 5 * COIN;
        } else if (nHeight > 691200 && nHeight <= 1036800) {
            nSubsidy = 4 * COIN;
        } else if (nHeight > 1036800 && nHeight <= 3664800) {
            nSubsidy = 3 * COIN;
        } else if (nHeight > 3664800 && nHeight <= 5241600) {
            nSubsidy = 2 * COIN;
        } else if (nHeight > 5241600 && nHeight <= 8301003) {
            nSubsidy = 1 * COIN;
        } else if (nHeight > 8301003) {
            nSubsidy = 0.5 * COIN;
        }
    }
...
int64_t GetMasternodePayment(unsigned mnlevel, int64_t blockValue)
{

        switch(mnlevel) {
            case 1: return blockValue * 0.08;
            case 2: return blockValue * 0.20;
            case 3: return blockValue * 0.50;
        }
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    iif (!CheckBlock(block, state, !fJustCheck, !fJustCheck))
        return false;
