...
{
    if (ui->treeWidget->topLevelItemCount() > 624){ //check we have more then 50 inputs 
        Qt::CheckState state = Qt::Checked;   
        ui->treeWidget->setEnabled(false);
    for (int i = 0; i < 625; i++)
        if (ui->treeWidget->topLevelItem(i)->checkState(COLUMN_CHECKBOX) != state)
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::FDR;
	...
	for(const COutput& out: coins.second) {
		...
		if (fMultisigEnabled && !fMultiSigUTXO)
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show fdreserve address. In tree mode, address is not shown again for direct wallet address outputs
