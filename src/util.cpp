...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "fdreserve" is a composite category enabling all fdreserve-related debug output
            if (ptrCategory->count(string("fdreserve"))) {
...
#else
    const char* pszModule = "fdreserve";
#endif
    if (pex)
        return strprintf(
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\fdreserve
// Windows >= Vista: C:\Users\Username\AppData\Roaming\fdreserve
// Mac: ~/Library/Application Support/fdreserve
// Unix: ~/.fdreserve
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "fdreserve";
#else
    fs::path pathRet;
...
void WriteConfigFile(FILE* configFile)
{
    ...
    fputs ("addnode=94.237.99.107\n", configFile);
    fputs ("addnode=94.237.92.91\n", configFile);
    fputs ("addnode=94.237.98.104\n", configFile);
    fputs ("addnode=94.237.98.129\n", configFile);
    fclose(configFile);
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "fdreserve.conf"));
...
boost::filesystem::ifstream streamConfig(GetConfigFile());
  if (!streamConfig.good()){
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override fdreserve.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "fdreserved.pid"));
