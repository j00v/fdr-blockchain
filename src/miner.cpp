...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("fdreserveMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("fdreserveMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("FDR Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fdr-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            ...
            if (Params().NetworkID() == CBaseChainParams::MAIN)
                while (chainActive.Tip()->nTime < Params().GenesisBlock().nTime || vNodes.empty() || !masternodeSync.IsSynced() || pwallet->IsLocked() || !fMintableCoins || nReserveBalance >= pwallet->GetBalance()) {
        ...
        //LogPrintf("Running fdreserveMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(), ::GetSerializeSize(*pblock, SER_NETWORK, PROTOCOL_VERSION));
